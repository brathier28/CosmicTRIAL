...
function love.load()
    ...
    blackHole = object.new(10000, vector.new(500, 500), vector.new(0, 0), 100)

    ball1 = ball.new(100, vector.new(500, 0), vector.new(-4.472, 0), 20)
...
function love.draw()
    if gameState == "start" then
        love.graphics.print("Welcome to Cosmic Devourer!", 230, 0, 0, 2, 2)
        love.graphics.print("Consume smaller planets to grow", 215, 35, 0, 2, 2)
        love.graphics.print("Press any key to start", 270, 500, 0, 2, 2)
        ---love.graphics.draw(love.graphics.newImage("mars.png"), 100, 200) --scale later
    end
    if (gameState == "running") or (gameState == "ending") then
        ...
        love.graphics.print("Avoid larger planets!", 100, 0, 0, 2, 2)
    ...
    if gameState == "gameover" then
        love.graphics.print("GAME OVER! Click anything to play again!", 250, 400, 0, 2, 2)
        --add fun fact(s) if time allows
    end
    if gameState == "winner" then
        love.graphics.setColor(0, 0, 1)
        love.graphics.print("Congrats! You are THE DEVOURER!", 250, 400, 0, 2, 2)
        --add fun fact(s) if time allows
    end
